...
(async function allOfArtist(){
    ...
    function resetConfig(){
        Spicetify.showNotification('All Of Artists settings restored.');
		Spicetify.LocalStorage.set("allOfArtist:settings", JSON.stringify(defaultConfig));
    }

    const defaultConfig = { addFeatures: true, addCompilations: true, trackPriority: 0, removeDupes: true, removeDupesConfirm: false, sortOrder: 0, inAppNotification: 0 };
...
function styleSettings() {
    ...
    function button(name, desc) {
        ...
        container.innerHTML = `
            <label class="col description">${desc}</label>
            <div class="col action"><button class="reset">${name}</button></div>
            `;
    ...
    content.appendChild(header("Local Storage"));
    content.appendChild(button("Restore", "Restore Settings to Default"));
	}
	...
	async function makePlaylist_getTracks(uris){
      ...
      if (CONFIG["inAppNotification"] == 0) {
			  Spicetify.showNotification('All Of '+artistData.name+' created.');
      }
      else if (CONFIG["inAppNotification"] == 1) { 
				...
				for(let r = 0; r < albumTracks.items.length; r++){
					for(let c = 0; c < albumTracks.items[r].artists.length; c++){
						if(albumTracks.items[r].artists[c].id == artistData.id && !(!CONFIG["addFeatures"] && c > 0)){
                            ...
                            trackInfo = {"name": albumTracks.items[r].name, "uri": albumTracks.items[r].uri, "trackCount": albumTracks.total, "type": array[i][2], "index": tracksAdd.length+"_"+albumTracksAdd.length};
                            if(index && CONFIG["removeDupes"]){
                                if(CONFIG["trackPriority"] == 0 && (array[i][2] != "compilation" && (tracks[index].type == "compilation" || albumTracks.total > tracks[index].trackCount))){
